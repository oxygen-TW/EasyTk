...
- [ScrollableTreeView](#scrollabletreeview)
...
```python
from EasyTk import BetterEntry, ScrollableTextArea, HyperlinkLabel, ScrollableTreeView
```
...
`ScrollableTreeView` 是基於 `ttk.Treeview` 的表格元件，內建捲軸，適合顯示清單與表格資料。

### 特色

- `orient` 支援垂直 / 水平 / 雙向捲動
- 預設 `show="headings"`，以欄位標題模式顯示
- 可透過 `insert_row()` 套用每列字型與樣式
- 提供 `load_csv()` 快速匯入 CSV

### 參數

- `orient`: `"vertical" | "horizontal" | "both"`
- 其餘 `ttk.Treeview` 原生參數可直接傳入

### 常用方法

- `set_header(header: list)`：設定欄位名稱
- `insert_row(values: list, font=None, **tag_conf)`：插入單列資料
- `load_csv(csv_file: str)`：由 CSV 載入資料（首列視為標題）

### 範例

```python
from EasyTk import ScrollableTreeView

tree = ScrollableTreeView(root, orient="both")
tree.pack(fill="both", expand=True)

tree.set_header(["Name", "Age", "City"])
tree.insert_row(["Alice", "30", "New York"], font={"family": "Arial", "size": 12, "weight": "bold"}, foreground="red")
...
- [ScrollableTextArea](#scrollabletextarea)
- [HyperlinkLabel](#hyperlinklabel)
- [ScrollableTreeView](#scrollabletreeview-1)
...
`ScrollableTreeView` is a table widget based on `ttk.Treeview` with built-in scrollbars, suitable for list/table-style data.

#### Features

- `orient` supports vertical / horizontal / both scrolling
- Uses `show="headings"` by default
- Supports per-row styling via `insert_row()`
- Provides `load_csv()` for quick CSV import

#### Parameters

- `orient`: `"vertical" | "horizontal" | "both"`
- Other native `ttk.Treeview` arguments are supported

#### Common Methods

- `set_header(header: list)`: set column names
- `insert_row(values: list, font=None, **tag_conf)`: insert one row
- `load_csv(csv_file: str)`: load data from CSV (first row as header)
